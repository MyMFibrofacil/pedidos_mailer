...
app = FastAPI(title="Pedidos Mailer", version="1.0.1")

# =========================
# FORZAR IPv4 (evita Errno 101 por IPv6 sin ruta)
# =========================
...
def _getaddrinfo_ipv4_first(host, port, family=0, type=0, proto=0, flags=0):
    ...
    return ipv4 if ipv4 else infos
...
def send_email_sendgrid(subject: str, html_body: str, to_email: str) -> None:
    api_key = os.getenv("SENDGRID_API_KEY")
    from_email = os.getenv("MAIL_FROM")
    from_name = os.getenv("MAIL_FROM_NAME", "Pedidos")

    if not api_key:
        raise RuntimeError("Falta SENDGRID_API_KEY en Render > Environment")
    if not from_email:
        raise RuntimeError("Falta MAIL_FROM en Render > Environment (sender verificado en SendGrid)")
...
def render_order_html(p: OrderPayload) -> str:
    ...
    for it in p.items:
        ...
        rows += f"""
        <tr>
          <td style="padding:8px;border-bottom:1px solid #ddd;">{it.producto}</td>
          <td style="padding:8px;border-bottom:1px solid #ddd;text-align:right;">{it.cantidad}</td>
    ...
    <div style="font-family:Arial,sans-serif;max-width:720px">
      ...
      <p style="font-size:12px;color:#666;">Generado automáticamente por el Asistente de Pedidos.</p>
    </div>
    """
...
def send_order_email(payload: OrderPayload):
    ...
    if not to_email:
        raise HTTPException(status_code=500, detail="Falta GMAIL_TO en Render > Environment")

    subject = f"Pedido {payload.order_id} - {payload.cliente_nombre}"
    html = render_order_html(payload)

    try:
        send_email_sendgrid(subject, html, to_email)
